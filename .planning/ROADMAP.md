...
- [x] **Phase 1: Turso Data Migration & Legacy Purge** - Extract existing historical data into DuckDB, purge Turso and cloud artifacts.
- [x] **Phase 2: DuckDB Storage Engine** - Build native DuckDB connection, time-series schema, batch inserts, and candle resampling.
- [x] **Phase 3: 24/7 Live WebSocket Streaming** - Persistent real-time streaming for Capital.com and Binance with auto-reconnect.
- [x] **Phase 4: Test Suite Migration & Verification** - End-to-end integration tests verifying live data write, DuckDB queries, and test suite.
...
**Success Criteria**:
  1. `src/database/connection.py` and `operations.py` manage DuckDB connections and initialize the `market_data` table schema.
